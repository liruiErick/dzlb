...
c.load();a.carouselItems.push(c)});
this.dom.carousel.on(this.events.startEvent,function(b){
	a.onStart(b)});
	SKY.Utils.hasTouchSupport()||this.dom.carousel.hover(function(b){
		a.mouseOver=!0;
		a.updateCursor()},function(b){a.mouseOver=!1;a.updateCursor()});
this.selectedItem=this.getStartItem();
this.selectedItem.addClass("sc-selected");


this.updatePlugin()},initGradientOverlays:function(){var a=this.settings;if(a.gradientOverlayVisible){var b=this.createGradientOverlay("left",a.gradientStartPoint,a.gradientEndPoint,
...
this.onClosestChanged(this.closestItem));a=null},select:function(a,b){var c=this.settings;if("number"===typeof a)var d=this.carouselItems[a];
else"object"===typeof a&&(d=a);
this.selectedItem&&this.selectedItem.removeClass("sc-selected");
d.addClass("sc-selected");
this.selectedItem=d;
c=this.selectedItem.getBaseOffset()+c.itemWidth/2+c.selectedItemDistance;this.container.setX(this.centerX-c,b);this.dom.carousel.trigger({type:"itemSelected.sc",item:this.selectedItem})},selectNext:function(a){var b=this.selectedItem.index();
